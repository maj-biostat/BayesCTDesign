...
#' Checks for Errors when Outcome is Poisson.
#'
#' \code{poisson_error_checks()} function used only used internally by
...
#'
#' @return \code{poisson_error_checks()} returns messages when
...
#' #None
#' @keywords internal
poisson_error_checks <- function(effect_vals, hist_control_data, rand_control_diff, alpha) {
    ...
    for (eff_val in effect_vals) {
        ...
        if (eff_val <= 0)
            stop("For Poisson, historic_sim requires effect_vals to represent Mean Ratios which must be positive numbers")
    }
...
#' Checks for Errors when Outcome is Poisson.
#'
#' \code{poisson_error_checks_simple()} function used only used internally by
...
#'
#' @return \code{poisson_error_checks_simple()} returns messages when
...
#' #None
#' @keywords internal
poisson_error_checks_simple <- function(effect_vals, control_parms, alpha) {
    ...
    for (eff_val in effect_vals) {
        ...
        if (eff_val <= 0)
            stop("For Poisson, simple_sim requires effect_vals to represent Mean Ratios which must be positive numbers")
    }

    if (chk_parm_lgth != 1) {
        stop("simple_sim requires a single elements in control_parms which is the lambda parameter, mean, for rpois()")
    }
    if (chk_parm_lgth == 1) {
        if (!is.numeric(control_parms[1]))
            stop("simple_sim requires Poisson Lambda parameter for controls to be numeric.")
        if (control_parms[1] <= 0)
            stop("simple_sim requires Poisson Lambda parameter for controls to be positive.")
    }
